...
//
// The canonical wire contract is maintained in the TowerOps server at
// priv/proto/agent.proto. Everything from `syntax = "proto3";` down must stay
// byte-for-byte identical to that copy; each repository keeps its own header.
...
// (IDENTIFY) GETs the system group only; the server classifies the device
// from sysObjectID/sysDescr, then issues phase two (WALK) carrying just the
// OID roots the matched profile needs. Agents built before this field existed
// report DISCOVERY_PHASE_UNSPECIFIED and the server falls back to the job id.
type DiscoveryPhase int32

const (
	DiscoveryPhase_DISCOVERY_PHASE_UNSPECIFIED DiscoveryPhase = 0
	DiscoveryPhase_DISCOVERY_PHASE_IDENTIFY    DiscoveryPhase = 1
	DiscoveryPhase_DISCOVERY_PHASE_WALK        DiscoveryPhase = 2
)
...
var (
	DiscoveryPhase_name = map[int32]string{
		...
		1: "DISCOVERY_PHASE_IDENTIFY",
		2: "DISCOVERY_PHASE_WALK",
	}
	DiscoveryPhase_value = map[string]int32{
		"DISCOVERY_PHASE_UNSPECIFIED": 0,
		"DISCOVERY_PHASE_IDENTIFY":    1,
		"DISCOVERY_PHASE_WALK":        2,
	}
)
...
func (x *AgentJob) GetIntervalSeconds() uint32 {
	if x != nil {
		return x.IntervalSeconds
	}
	return 0
...
func (x *AgentJob) GetDiscoveryPhase() DiscoveryPhase {
	if x != nil {
		return x.DiscoveryPhase
	}
	return DiscoveryPhase_DISCOVERY_PHASE_UNSPECIFIED
}

func (x *AgentJob) GetCredentialProbe() *CredentialProbe {
	if x != nil {
		return x.CredentialProbe
	}
	return nil
}
...
type CredentialTestResult struct {
	...
	Timestamp         int64                  `protobuf:"varint,5,opt,name=timestamp,proto3" json:"timestamp,omitempty"`
	SysObjectId       string                 `protobuf:"bytes,6,opt,name=sys_object_id,json=sysObjectId,proto3" json:"sys_object_id,omitempty"` // sysObjectID.0 value if success; maximum 255 bytes
	SysName           string                 `protobuf:"bytes,7,opt,name=sys_name,json=sysName,proto3" json:"sys_name,omitempty"`               // sysName.0 value if success; maximum 255 bytes
	unknownFields     protoimpl.UnknownFields
	sizeCache         protoimpl.SizeCache
}
...
func (x *CredentialTestResult) GetSysObjectId() string {
	if x != nil {
		return x.SysObjectId
	}
	return ""
...
func (x *CredentialTestResult) GetSysName() string {
	if x != nil {
		return x.SysName
	}
	return ""
...
// A credential probe tries each candidate in order against one target and
// reports which one answered, so the server can pin the working credential
// set on the device and classify the probed host from its system values.
// Each candidate carries the target IP; the server builds all candidates for
// one probe against the same address.
//
// The agent truncates the three system strings to the bounds below before
// sending - the server rejects over-length values and drops the result.
type CredentialProbe struct {
	state               protoimpl.MessageState `protogen:"open.v1"`
	Candidates          []*SnmpDevice          `protobuf:"bytes,1,rep,name=candidates,proto3" json:"candidates,omitempty"`                                                   // Ordered; the agent tries them in sequence
	StopOnFirstSuccess  bool                   `protobuf:"varint,2,opt,name=stop_on_first_success,json=stopOnFirstSuccess,proto3" json:"stop_on_first_success,omitempty"`    // Stop after the first candidate that answers
	AttemptTimeoutMs    uint32                 `protobuf:"varint,3,opt,name=attempt_timeout_ms,json=attemptTimeoutMs,proto3" json:"attempt_timeout_ms,omitempty"`            // Per-attempt timeout; zero means the agent default
	AttemptRetries      uint32                 `protobuf:"varint,4,opt,name=attempt_retries,json=attemptRetries,proto3" json:"attempt_retries,omitempty"`                    // Extra attempts per candidate; zero means one attempt
	InterAttemptDelayMs uint32                 `protobuf:"varint,5,opt,name=inter_attempt_delay_ms,json=interAttemptDelayMs,proto3" json:"inter_attempt_delay_ms,omitempty"` // Delay between attempts; the agent enforces a 250 ms floor
	unknownFields       protoimpl.UnknownFields
	sizeCache           protoimpl.SizeCache
}
...
func (x *CredentialProbe) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *CredentialProbe) GetStopOnFirstSuccess() bool {
	if x != nil {
		return x.StopOnFirstSuccess
	}
	return false
}

func (x *CredentialProbe) GetAttemptTimeoutMs() uint32 {
	if x != nil {
		return x.AttemptTimeoutMs
	}
	return 0
}

func (x *CredentialProbe) GetAttemptRetries() uint32 {
	if x != nil {
		return x.AttemptRetries
	}
	return 0
}

func (x *CredentialProbe) GetInterAttemptDelayMs() uint32 {
	if x != nil {
		return x.InterAttemptDelayMs
	}
	return 0
}
...
type CredentialProbeResult struct {
	state         protoimpl.MessageState `protogen:"open.v1"`
	ProbeId       string                 `protobuf:"bytes,1,opt,name=probe_id,json=probeId,proto3" json:"probe_id,omitempty"`                 // The job_id of the CREDENTIAL_PROBE job
	MatchedIndex  int32                  `protobuf:"varint,2,opt,name=matched_index,json=matchedIndex,proto3" json:"matched_index,omitempty"` // Index into candidates that answered; -1 when none did
	SysObjectId   string                 `protobuf:"bytes,3,opt,name=sys_object_id,json=sysObjectId,proto3" json:"sys_object_id,omitempty"`   // sysObjectID.0 from the matched candidate; maximum 255 bytes
	SysDescr      string                 `protobuf:"bytes,4,opt,name=sys_descr,json=sysDescr,proto3" json:"sys_descr,omitempty"`              // sysDescr.0 from the matched candidate; maximum 2,048 bytes
	SysName       string                 `protobuf:"bytes,5,opt,name=sys_name,json=sysName,proto3" json:"sys_name,omitempty"`                 // sysName.0 from the matched candidate; maximum 255 bytes
	ErrorMessage  string                 `protobuf:"bytes,6,opt,name=error_message,json=errorMessage,proto3" json:"error_message,omitempty"`  // Last attempt's error when matched_index is -1
	Timestamp     int64                  `protobuf:"varint,7,opt,name=timestamp,proto3" json:"timestamp,omitempty"`
	DurationMs    uint32                 `protobuf:"varint,8,opt,name=duration_ms,json=durationMs,proto3" json:"duration_ms,omitempty"` // Agent-measured wall time for the whole probe
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *CredentialProbeResult) String() string {
	return protoimpl.X.MessageStringOf(x)
}
...
func (x *CredentialProbeResult) GetMatchedIndex() int32 {
	if x != nil {
...
func (x *CredentialProbeResult) GetSysObjectId() string {
	if x != nil {
		return x.SysObjectId
	}
	return ""
...
func (x *CredentialProbeResult) GetSysDescr() string {
	if x != nil {
		return x.SysDescr
	}
	return ""
...
func (x *CredentialProbeResult) GetSysName() string {
	if x != nil {
		return x.SysName
	}
	return ""
...
func (x *CredentialProbeResult) GetErrorMessage() string {
	if x != nil {
		return x.ErrorMessage
	}
	return ""
...
func (x *CredentialProbeResult) GetTimestamp() int64 {
	if x != nil {
		return x.Timestamp
	}
	return 0
...
func (x *CredentialProbeResult) GetDurationMs() uint32 {
	if x != nil {
		return x.DurationMs
	}
	return 0
...
const file_proto_agent_proto_rawDesc = "" +
	...
	"\ttimestamp\x18\x04 \x01(\x03R\ttimestamp\"\xfa\x01\n" +
	...
	"\rsys_object_id\x18\x06 \x01(\tR\vsysObjectId\x12\x19\n" +
	"\bsys_name\x18\a \x01(\tR\asysName\"\x8c\x02\n" +
	"\x0fCredentialProbe\x12:\n" +
	"\n" +
	"candidates\x18\x01 \x03(\v2\x1a.towerops.agent.SnmpDeviceR\n" +
	"candidates\x121\n" +
	"\x15stop_on_first_success\x18\x02 \x01(\bR\x12stopOnFirstSuccess\x12,\n" +
	"\x12attempt_timeout_ms\x18\x03 \x01(\rR\x10attemptTimeoutMs\x12'\n" +
	"\x0fattempt_retries\x18\x04 \x01(\rR\x0eattemptRetries\x123\n" +
	"\x16inter_attempt_delay_ms\x18\x05 \x01(\rR\x13interAttemptDelayMs\"\x97\x02\n" +
	"\x15CredentialProbeResult\x12\x19\n" +
	"\bprobe_id\x18\x01 \x01(\tR\aprobeId\x12#\n" +
	"\rmatched_index\x18\x02 \x01(\x05R\fmatchedIndex\x12\"\n" +
	"\rsys_object_id\x18\x03 \x01(\tR\vsysObjectId\x12\x1b\n" +
	"\tsys_descr\x18\x04 \x01(\tR\bsysDescr\x12\x19\n" +
	"\bsys_name\x18\x05 \x01(\tR\asysName\x12#\n" +
	...
	"\x1bDISCOVERY_PHASE_UNSPECIFIED\x10\x00\x12\x1c\n" +
	"\x18DISCOVERY_PHASE_IDENTIFY\x10\x01\x12\x18\n" +
	"\x14DISCOVERY_PHASE_WALK\x10\x02B+Z)github.com/towerops-app/towerops-agent/pbb\x06proto3"
